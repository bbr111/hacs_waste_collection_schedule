...
| bbd-whitespace Drupal module (UK) | `collection_dates_retriever(site)`, `CollectionDatesParser`, `TYPE_VALUE_MAP` | A council's own `/bbd-whitespace/one-year-collection-dates?uprn=...` endpoint (Hart, Erewash): a Drupal AJAX command list carrying a year of collections grouped by month. Adding a council is its site URL. |

...
TITLE = "Heidelberg"
DESCRIPTION = (
    "Support for the waste collection schedule provided by the Office of Waste Management and Municipal Cleansing Heidelberg"
)
...
TEST_CASES = {
    ...
    "No weekly collection of residual and bio waste possible": {"street": "Molkenkurweg"},
...
class WasteInformation:
    ...
    def _exclude_weekdays_without_collection(self):
        ...
        if self._collection_schedule == SCHEDULE_TYPES["uneven_weeks"]:
            weekdays_without_collection = list(filter(lambda x: int(x.strftime("%W")) % 2 == 1, self._collection_ruleset))

        if self._collection_schedule == SCHEDULE_TYPES["even_weeks"]:
            weekdays_without_collection = list(filter(lambda x: int(x.strftime("%W")) % 2 == 0, self._collection_ruleset))
...
class Source:
    ...
    def _extract_collection_data(self, raw_collection_data):
        ...
        for waste_type in WASTE_TYPES:
            if waste_type == WASTE_TYPES["christmas"]:
                collection_date = date_parser.parse(raw_collection_data[waste_type][0]["collection_date"], ignoretz=True)
                ruleset = rruleset()
                ruleset.rdate(collection_date)

                collection_data.append(WasteInformation(waste_type, None, None, ruleset))
            else:
                if waste_type == WASTE_TYPES["rest"] and self._collect_residual_waste_weekly is False:
                    collection_schedule = (
                        SCHEDULE_TYPES["even_weeks"] if self._even_house_number is True else SCHEDULE_TYPES["uneven_weeks"]
                    )
                else:
    ...
    def fetch(self) -> list[Collection]:
        ...
        if len(raw_collection_data["collections"]) == 0:
            raise SourceArgumentNotFoundWithSuggestions("street", self._street, self._get_available_streets())

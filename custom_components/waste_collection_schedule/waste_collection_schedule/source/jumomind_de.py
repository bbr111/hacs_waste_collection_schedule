...
def EXTRA_INFO():
    ...
    for provider, entries in SERVICE_MAP.items():
        ...
        for area in entries["list"]:
            ...
            extra_info.append({"title": title, "url": url, "default_params": {"service_id": provider}})
    return extra_info
...
def normalize_street(value: str | None) -> str | None:
    return value and (value.lower().strip().casefold().replace("straße", "strasse").replace("str.", "strasse"))
...
class Source:
    def __init__(
        ...
        self._house_number: str | None = str(house_number).lower().strip().lstrip("0") if house_number else None
    ...
    def fetch(self):
        ...
        if city_id is None and self._city is None:
            raise SourceArgumentExceptionMultiple(["city", "city_id"], "City or city id is required")
        if city_id is not None and self._city is not None:
            raise SourceArgumentExceptionMultiple(["city", "city_id"], "City OR city id is required. Do not use both")
        ...
        if city_id is not None:
            ...
            if city_id is None:
                raise SourceArgumentNotFoundWithSuggestions("city", self._city, [c["name"] for c in cities])

            if has_streets:
                r = session.get(self._api_url, params={"r": "streets", "city_id": city_id})
                r.raise_for_status()
                streets = r.json()

                street_found = False
                for street in streets:
                    if normalize_street(street["name"]) == normalize_street(self._street) or normalize_street(
                        street["_name"]
                    ) == normalize_street(self._street):
                        street_found = True
                        area_id = street["area_id"]
                        if "houseNumbers" in street:
                            for house_number in street["houseNumbers"]:
                                if house_number[0].lower().strip().lstrip("0") == self._house_number:
                ...
                if not street_found:
                    ...
                    raise SourceArgumentNotFoundWithSuggestions("street", self._street, streets_suggestions)
            else:

...
class Source:
    ...
    def fetch(self):
        ...
        response = requests.get(f"{data_api}/PickupSchedules/findbuilding/{self._street_address}")
        ...
        for item in data:
            ...
            if res:
                next_pickup_date = datetime.fromisocalendar(year=int(res.group(2)), week=int(res.group(1)), day=1).date()
            else:

...
class Source:
    ...
    def fetch(self):
        ...
        bin_collection_info_page = self.__get_bin_collection_info_page(session, address_page, self._postcode, self._uprn)
        ...
        if ical_content is not None:
            ...
            if webpage_content is not None:
                ...
                for d in collections:
                    ...
                    entries.append(
                        Collection(datetime.strptime(d["nextCollectionISO"], "%Y-%m-%d").date(), d["binType"], icon=icon)
                    )
    ...
    def __get_ical_bin_collection_info(self, bin_collection_info_page):
        ...
        if not script:
            raise Exception("no script tag cannot find WLBINCOLLECTIONSerializedVariables")

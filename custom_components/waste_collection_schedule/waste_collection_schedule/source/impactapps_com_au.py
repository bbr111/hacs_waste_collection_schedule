...
TITLE = "Impact Apps"
DESCRIPTION = "Source for councils using Impact Apps (waste-info.com.au) for waste collection."
...
class LocationFinder:
    ...
    def find_suburb_id(self, session: requests.Session, suburb: str) -> int:
        ...
        suburb_id = next((item["id"] for item in suburbs if item["name"] == suburb), None)
        if suburb_id is None:
            raise SourceArgumentNotFoundWithSuggestions("suburb", suburb, [item["name"] for item in suburbs])
        return suburb_id
    ...
    def find_street_id(self, session: requests.Session, suburb_id: int, street_name: str) -> int:
        ...
        street_id = next((item["id"] for item in streets if item["name"] == street_name), None)
        if street_id is None:
            raise SourceArgumentNotFoundWithSuggestions("street_name", street_name, [item["name"] for item in streets])
        return street_id
    ...
    def find_property_id(
        ...
        property_id = next(
            (item["id"] for item in properties if item["name"] == f"{street_number} {street_name} {suburb}"),
            None,
        )
...
class Source:
    ...
    def fetch(self) -> list[Collection]:
        ...
        if not self.property_id:
            ...
            street_id = self.location_finder.find_street_id(session, suburb_id, self.street_name)
        ...
        response = session.get(url, params={"start": start_date.isoformat(), "end": end_date.isoformat()})
        ...
        for event in events:
            ...
            if is_recurring:
                ...
                collection_dates = generate_recurring_dates(recurring_event, start_date, end_date)
            else:

...
def test_past_date_moves_forward(monkeypatch):
    ...
    assert results == expected, (
        f"Expected dates {expected} for start_date={start_date}, n={n}, delta={delta}, but got {results}"
    )
...
def test_start_date_is_today(monkeypatch):
    ...
    results = darebin_vic_gov_au._get_next_n_dates(start_date, n, delta)
    assert results == expected, f"Expected {expected} for start_date={start_date}, n={n}, delta={delta}, but got {results}"
...
def test_get_previous_date_for_day_of_week_monday(monkeypatch):
    ...
    assert results == expected, f"Expected {expected} for day_of_week={collection_day}, but got {results}"
